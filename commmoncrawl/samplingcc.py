...
# increase CSV field limit to handle very large article text cells
...
max_per_source = 80  # cap on articles collected per source per year
years = [2024]       # years to process

# Common Crawl index collections for 2024, one per crawl window
cc_2024 = [
...
def load_keywords(path):
    # read one keyword per line, skipping blank lines
    with open(path, "r", encoding="utf-8") as f:
        return [line.strip() for line in f if line.strip()]


def build_automaton(words):
    # build an Aho-Corasick automaton for fast multi-keyword matching.
    # much faster than looping over keywords individually for each URL/text.
...
def load_sources(path):
    # read the source list CSV, keeping only the columns we need
    df = pd.read_csv(path)
    return df[["name", "state", "city", "URL"]].dropna()


def query_cc(domain, year, retries=3, delay=4):
    # query the Common Crawl CDX index for all pages under a domain in a given year.
    # tries each crawl collection for that year, with exponential backoff on failure.
    # returns a flat list of CDX records (dicts with url, filename, offset, length, etc.)

    if year == 2024:
        collections = cc_2024
    else:
        # map older years to the closest available end-of-year crawl
        year_map = {
        ...
        collections = [year_map[year]] if year in year_map else []
    ...
    for col in collections:
        ...
        for attempt in range(1, retries + 1):
            try:
                ...
                # CDX API returns one JSON object per line, not a JSON array
...
def domain_variants(domain):
    # generate plausible subdomain variants for a domain.
    # Common Crawl may have indexed content under www., m., or editorial subdomains
    # that won't appear under the bare domain query.
...
def url_matches(url, ac):
    # return the set of keywords that appear in the URL string.
    # uses the pre-built automaton for O(n) matching.
    return list({match for _, (_, match) in ac.iter(url.lower())})


def extract_text(html):
    # extract readable article body text from raw HTML.
    # tries semantic tags first (<article>), then known publisher div classes,
    # then falls back to <body>. returns empty string if nothing substantial found.
    ...
    # prefer the <article> tag — most news sites use it for the story body
    article = soup.find("article")
    if article and len(article.get_text(strip=True)) > 1000:
        return article.get_text(separator="\n", strip=True)

    # fall back to known publisher-specific div classes
    div_classes = [
        "content__article-body", "article-body", "l-container",
...
def extract_warc(record):
    # fetch a single WARC record from Common Crawl using a byte-range request.
    # each CDX record points to an exact byte offset in a WARC file on S3,
    # so we can download just that article without fetching the whole file.
    # returns extracted text if >1000 chars, otherwise None.
...
def read_last_years(path):
    # parse the output txt file to find the most recent year processed per domain.
    # used to skip re-processing domains we've already crawled for a given year.
    # returns a dict mapping domain → most recent year seen.
    last = {}
    ...
    # each article block starts with "---", followed by url: and year: lines
...
if __name__ == "__main__":
    ...
    keywords_path = "/content/drive/MyDrive/keywords.txt"  # one keyword per line
    ...
    with open(results_csv, "a", newline="", encoding="utf-8") as csvfile, \
        ...
        for _, row in tqdm(sources.iterrows(), total=len(sources)):
            ...
            for year in years:
                # skip if we already collected articles for this domain/year
                ...
                for variant in domain_variants(domain):
                    ...
                    for rec in records:
                        ...
                        # only fetch full content if the URL itself contains a keyword
                        ...
                        # write matched article to both output files
